function [] = PhaseTransitionForLocalizedChimeraVsPhaseLag
    ...
    size_of_real = 8;
    folder = '/Users/nikita/Documents/Projects/spc2/spc2FiniteVolumeMethods/Continuation/ContinuationPhaseLagFromNonhomogeneous/right_path/';
    folder_info = dir(folder);
    is_simulation_file = contains({folder_info(:).name}, '.bin')';
    phase_lag_coefficients = zeros(size(folder_info));
    ...
    for i = 1 : size(folder_info, 1)
        if is_simulation_file(i)
            ...
            phase_lag_coefficients(i, 1) = sscanf(base_name(key_index+length(key) : end), '%g', 1);
            ...
            for tau = 1 : t_aver
                ...
                spatial_order_parameters(i, 1) = spatial_order_parameters(i, 1) + SpatialOrderParameter(continuum_limit_pdf, n, m, l, dx, dy, dphi, circular_mesh_x, circular_mesh_y) / t_aver;
            end % tau
        end
    end % i
    phase_lag_coefficients = phase_lag_coefficients(is_simulation_file);
    spatial_order_parameters = spatial_order_parameters(is_simulation_file);
    [phase_lag_coefficients, sort_idx] = sort(phase_lag_coefficients);
    spatial_order_parameters = spatial_order_parameters(sort_idx);
    
    figure;
    hold on;
    set(gca, 'ColorOrderIndex', 1);
    plot(phase_lag_coefficients, spatial_order_parameters, 'o-', 'LineWidth', 1, 'MarkerSize', 8);
%     xlim([1.3, 1.555]);
    box on;
    set(gca,...
        'Units', 'Normalized',...
...
%     phase_lag = 1.435 : 0.005 : 1.515;
%     plot(phase_lag, phase_lag.*0, '--k', 'LineWidth', 1);
%     ylim([-0.03, 0.36]);
%     LoadSolutionOfSelfconistentEquations();
%     ylim([0.3, 1.03]);
     % Plot an initial point of continuation method
%     order_parameter_value = 0.6896;%0.6896;%0.1107;
%     plot(1.45, order_parameter_value, '*k', 'MarkerSize', 8, 'LineWidth', 1);
%     plot(1.45, order_parameter_value, 'ok', 'MarkerSize', 8, 'LineWidth', 1);
...
function [] = LoadSolutionOfSelfconistentEquations()
    ...
    phase_lag_max = 1.515;
    diffusion = 0.0075;
    order_parameters = order_parameters(abs(order_parameters(:, 2) / nondimensionalized_sigma - diffusion) < 1e-4, :);
    order_parameters = order_parameters(order_parameters(:, 3) >= phase_lag_min & order_parameters(:, 3) <= phase_lag_max, :);
    hold on;
    plot(order_parameters(:, 3), order_parameters(:, 4), '--k', 'LineWidth', 1);
