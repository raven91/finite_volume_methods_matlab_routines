function [] = ErrorConvergenceForNonstationaryPhaseSynchronization3D
    ...
    t_0 = 0; 
    t_1 = t_0;
    folder = '/Users/nikita/Documents/Projects/spc2/spc2FiniteVolumeMethods/TestCaseNonstationaryPhaseSynchronization3D/';
    size_of_float = 8;
    ...
    for t_0 = 0 : 0.1 : t_0 + 0
        ...
        figure(1); hold on;
        plot(grid_points - pi, circshift(exact_density, - size(grid_points, 2) / 2 - 1), 'LineWidth', 2.5, 'Color', [0 0 0]);
...
%% f is 1 x nw
function [grid_point, f] = ExactSolution(t)
    ...
    w_max = 2 * pi;
    dw = (w_max - w_min) / nw;
    grid_point = w_min + (0 : 1 : nw - 0) * dw;
    grid_point_weight = grid_point .* 0 + dw;
...
%     grid_point_weight = grid_point .* 0 + dw / 2;
    ...
    jw = (0 : nw - 1);
    f_norm = vpa(sum(grid_point_weight(1, jw + 1) .* E(grid_point(1, jw + 1)) .* (1.0 + (c_e - 1.0) * IntegralUpToCurrentPoint(1, jw + 1) / I_2pi), 2));
    f = vpa(E(grid_point(1, jw + 1)) .* (1.0 + (c_e - 1.0) * IntegralUpToCurrentPoint(1, jw + 1) / I_2pi) / f_norm);
    grid_point = grid_point(1, jw + 1);
